...
export default function AdminPage() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#060d1a',fontFamily:'system-ui,sans-serif'}}>

      {/* Top Nav */}
      <div style={{background:'#0a1628',borderBottom:'1px solid rgba(255,255,255,.08)',padding:'0 1.5rem',position:'sticky',top:0,zIndex:100}}>
        <div style={{maxWidth:960,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',height:52}}>
          <div style={{display:'flex',alignItems:'center',gap:20}}>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              ...
              <span style={{fontSize:14,fontWeight:600,color:'white'}}>Admin</span>
            ...
            <nav style={{display:'flex',gap:2}}>
              {tabs.map(t => (
                <button key={t} onClick={() => setTab(t)} style={{
                  ...
                  background: tab === t ? 'rgba(37,99,235,.3)' : 'transparent',
                  color: tab === t ? '#60a5fa' : 'rgba(255,255,255,.5)',
          ...
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            <span style={{fontSize:11,color:'rgba(255,255,255,.3)'}}>July 29, 2026</span>
            <button onClick={loadStats} disabled={statsLoading} style={{fontSize:11,padding:'4px 10px',borderRadius:6,border:'1px solid rgba(255,255,255,.1)',background:'transparent',color:'rgba(255,255,255,.6)',cursor:'pointer'}}>
              {statsLoading ? '...' : '↻ Refresh'}
            </button>
            <a href="/dashboard/index.html" style={{fontSize:11,padding:'4px 10px',borderRadius:6,border:'1px solid rgba(255,255,255,.1)',color:'rgba(255,255,255,.4)',textDecoration:'none'}}>← Dashboard</a>
          </div>
        </div>
      </div>
        ...
        {tab === 'overview' && (
          <div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:16}}>
              {[
                { label:'Users', value: s.totalUsers ?? 0, sub:`+${s.newUsersThisWeek ?? 0} this week`, border:'#2563eb', color:'#60a5fa', icon:'👥' },
                { label:'Circles', value: s.totalCircles ?? 0, sub:`${s.activeCircles ?? 0} active`, border:'#10b981', color:'#34d399', icon:'⭕' },
                { label:'Revenue', value:`$${(stripe.monthRevenue||0).toFixed(2)}`, sub:'this month', border:'#f59e0b', color:'#fbbf24', icon:'💰' },
                { label:'Emails today', value: ses.emailsSent24h ?? 0, sub:`${emailsLeft.toLocaleString()} left`, border:'#8b5cf6', color:'#a78bfa', icon:'📧' },
              ].map(c => (
                <div key={c.label} style={{...card, borderTop:`2px solid ${c.border}`}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:6}}>
                    <span style={{fontSize:11,color:'rgba(255,255,255,.4)',textTransform:'uppercase',letterSpacing:'.5px'}}>{c.label}</span>
                    <span style={{fontSize:16}}>{c.icon}</span>
                  </div>
                  <div style={{fontSize:28,fontWeight:700,color:c.color,marginBottom:4}}>{statsLoading ? '...' : c.value}</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:16}}>
              <div style={{...card, borderTop:'2px solid #10b981'}}>
                <p style={{fontSize:12,fontWeight:600,color:'rgba(255,255,255,.6)',margin:'0 0 12px'}}>💰 Stripe this month</p>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:10}}>
                  {[
                    ...
                    <div key={item.label} style={{textAlign:'center'}}>
                      <div style={{fontSize:18,fontWeight:700,color:'#34d399'}}>{statsLoading ? '...' : item.value}</div>
                      <div style={{fontSize:10,color:'rgba(255,255,255,.3)',marginTop:2}}>{item.label}</div>
                    </div>
                  ))}
                </div>
                <a href="https://dashboard.stripe.com" target="_blank" rel="noopener noreferrer" style={{display:'block',textAlign:'center',fontSize:11,color:'#60a5fa',textDecoration:'none'}}>Open Stripe →</a>
              </div>

              <div style={{...card, borderTop:'2px solid #2563eb'}}>
                <p style={{fontSize:12,fontWeight:600,color:'rgba(255,255,255,.6)',margin:'0 0 12px'}}>📧 Email quota</p>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:12,marginBottom:6}}>
                  <span style={{color:'rgba(255,255,255,.5)'}}>{ses.sentLast24Hours||0} used today</span>
                  <span style={{color:'white',fontWeight:600}}>{emailsLeft.toLocaleString()} left</span>
                </div>
                <div style={{height:6,background:'rgba(255,255,255,.08)',borderRadius:100,overflow:'hidden',marginBottom:10}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8}}>
                  {[
                    ...
                    <div key={item.label} style={{textAlign:'center'}}>
                      <div style={{fontSize:16,fontWeight:700,color:'#60a5fa'}}>{statsLoading ? '...' : item.value}</div>
                      <div style={{fontSize:10,color:'rgba(255,255,255,.3)',marginTop:2}}>{item.label}</div>
                    </div>
                  ))}
                </div>
                <div style={{textAlign:'center',marginTop:8,fontSize:10,color:'#34d399'}}>✓ Production · 50,000/day limit</div>
              </div>
            </div>

            <div style={card}>
              <p style={{fontSize:12,fontWeight:600,color:'rgba(255,255,255,.6)',margin:'0 0 12px'}}>🔧 Platform info</p>
              <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:'4px 24px',fontSize:12}}>
                {[
                  ...
                  <div key={info.label} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid rgba(255,255,255,.04)'}}>
                    <span style={{color:'rgba(255,255,255,.4)'}}>{info.label}</span>
                    <span style={{color:'white',fontFamily:'monospace',fontSize:11}}>{info.value}</span>
                  </div>
                ))}
              </div>
            </div>
