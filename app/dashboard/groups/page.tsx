...
export default function GroupPage() {
  ...
  const loadData = async (id: string) => {
    try {
      ...
      // Load members
      console.log('Fetching members:', `${API_URL}/members?circleId=${id}`)
  ...
  const progress = circle.goal ? Math.min((parseFloat(circle.totalSaved || '0') / parseFloat(circle.goal)) * 100, 100) : 0
